...
extension Plot {
    ...
    public func drawGraph(renderer: Renderer = SVGRenderer()) -> String {
        drawGraph(size: Size(width: 1000, height: 750), renderer: renderer)
        return (renderer as! SVGRenderer).svg
    }
    
    /// Draws and saves the graph to the named file.
    /// - note: This function changes the `imageSize` of the `Renderer` it is given.
    public func drawGraphAndOutput(size: Size = Size(width: 1000, height: 750),
    ...
    /// - note: This function changes the `imageSize` of the `Renderer` it is given.
    public func drawGraphOutput(size: Size = Size(width: 1000, height: 750),
                                fileName name: String = "swiftplot_graph", renderer: Renderer = SVGRenderer()) throws {
        renderer.imageSize = size
        try renderer.drawOutput(fileName: name)
    }
